...
class LoadLevel(Menu):
    ...
    def setup_buttons_form(self,path):
        for button_name in self.setup_menu:
            if button_name == "level_1":
                button = Button(main_screen=self.main_screen,w=260,h=50,x=(RESOLUTION_WIDTH/2)-130,y=200,path=path,image_name=button_name,action=self.button_action,name=button_name)
                self.button_dict[button_name] = button
            if button_name == "level_2":
